...
def init_coords_and_images(width, height, player1_is_alive, player2_is_alive, window_width, window_height):
    ...
    # to avoid relative path fuckery
    dirname = os.path.dirname(__file__)
    os.path.join(dirname, "../resources/gameElements/python.png")
    ...
    for i in range(10):
        image_tile.append(
            pygame.transform.smoothscale(pygame.image.load(os.path.join(dirname,"../resources/gameElements/tile" + str(i + 1) + ".png")),
                                         (scale, scale)))
    tiles = []
    for i in range(width):
        tiles.append([])
